...
export class ProductValidation {
    description(value, resolve, reject, productRepository: ProductRepository, idsException: number[] = []) {
        const statusValidateNotEmpty = typeof value != "undefined" && value != null && (typeof value != "string" || value.length > 0);
        if (statusValidateNotEmpty) productRepository.findByDescription(value, idsException).then(valueCategory => {
